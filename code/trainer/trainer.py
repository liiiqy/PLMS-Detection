...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch, total_epochs):
        ...
        # writer1 = SummaryWriter('./runs/acc/train')
        # writer2 = SummaryWriter('./runs/acc/test')
        # writer3 = SummaryWriter('./runs/acc/all')
        writer4 = SummaryWriter('./runs/f1')
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader):
            ...
            N = list(output.size())
            output=torch.reshape(output,(N[0]*N[2],1))
            output = output.view(-1, N[0]*N[2])

            target=torch.reshape(target,(N[0]*N[2],1))
            target = target.view(-1,N[0]*N[2]) 
        ...
        # writer1.add_scalar('train_f1',log["u_f1"],epoch)

        if self.do_validation:
            val_log, outs, trgs = self._valid_epoch(epoch)
            log.update(**{'val_' + k: v for k, v in val_log.items()})
            if val_log["u_f1"] > self.selected:
                self.selected = val_log["u_f1"]
                selected_d["outs"] = outs
                selected_d["trgs"] = trgs
                # obj = self.model.state_dict()
            if epoch == total_epochs:
                overall_outs.extend(selected_d["outs"])
                overall_trgs.extend(selected_d["trgs"])
            # writer2.add_scalar('test_f1',val_log["u_f1"],epoch)

            writer4.add_scalars('f1', {'train': log["u_f1"], 'test': val_log["u_f1"]}, epoch)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target) in enumerate(self.valid_data_loader):
                ...
                outs = np.append(outs, output.cpu().numpy())
